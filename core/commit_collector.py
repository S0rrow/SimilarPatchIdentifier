...
def main(argv):
    ...
    if is_direct_input == False:
        input_df = pd.read_csv(root+"/components/commit_collector/Defects4J_bugs_info/input.csv", names=["Project","Faulty file path","faulty line","buggy sha","url","dummy"])
    ...
    if is_D4J:
        ...
        ## 미리 준비한 데이터에서 읽어오기
        d4j_input_df = pd.read_csv(root+"/components/commit_collector/Defects4J_bugs_info/"+project+".csv", names=["DefectsfJ ID","Faulty file path","fix faulty line","blame faulty line","dummy"])
    ...
    with open(output_dir+"/commit_collector/BFIC.csv", 'w', newline='') as csvfile:
        ...
        print(f"[debug.log] FIC SHA: {FIC_sha}")
        print(f"[debug.log] BFIC SHA: {BFIC_sha}")
