...
def main(argv):
    ...
    ### for D4J projects
    if is_D4J == True:
        assert copy(f"{root}/core/confix/coverages/{target_project.lower()}/{target_project.lower()}{target_id}b/coverage-info.obj", f"{target_dir}/")

        cdir = f"{target_dir}/"
        copy(f"{just_target}/properties/confix.properties", f"{target_dir}/")
        with open(f"{target_dir}/confix.properties", "a") as confix_prop_file:
            for d4j_prop, confix_prop in (("dir.src.classes", "src.dir"), ("dir.bin.classes", "target.dir"), ("dir.src.tests", "test.dir"), ("cp.compile", "cp.compile"), ("cp.test", "cp.test")):
                ...
                subprocess.run(["defects4j", "export", "-p", d4j_prop, "-o", filename], cwd = cdir)
                with open(f"{cdir}/{filename}", "r") as f:
                    line = f.read()
                    confix_prop_file.write(f"{confix_prop}={line}\n")

            confix_prop_file.write(f"projectName={target_project}\n")
        ...
        for test_list in ("tests.all", "tests.relevant", "tests.trigger"):
            outfile = f"{target_dir}/{test_list}"
            subprocess.run(["defects4j", "export", "-p", test_list, "-o", outfile], cwd = cdir)
    ...
    if not os.path.isfile(f"{target_dir}/patches/0/{perfect_faulty_path}"):
        ...
        with open(f"{just_target}/diff_file.txt", "w") as f:
            f.write(f"differences made in {perfect_faulty_path}:\n")

        with open(f"{just_target}/diff_file.txt", "a") as f:
            home_dir = os.path.expanduser('~')
            subprocess.run(["git", "diff", f"{target_dir}/{perfect_faulty_path}", f"{target_dir}/patches/0/{perfect_faulty_path}"], cwd = home_dir, stdout = f)

        with open(f"{just_target}/diff_file.txt", "r") as f:
            print(f.read())
